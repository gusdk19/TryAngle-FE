...
export default function ChallengeDetail() {
  ...
  const [loading, setLoading] = useState(
    challenge && !isLoggedIn ? false : true
  );
  ...
  const [participate, setParticipate] = useState(
    updatedStatus !== undefined ? updatedStatus : 0
  );
  ...
  useEffect(() => {
    ...
    const getUserChallengeData = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/challenge/my/${challengeId}`, {
          method: "GET",
          headers: {
            // 'Content-Type': 'application/json',
  ...
  return (
    <div className="bg-white flex flex-row justify-center w-full">
      <div className="bg-white w-[393px] h-[852px] relative">
        {/* Header */}
        <Header
          title={challengeData.challenge_name}
          page={page}
          prevPage={prevPage}
          id={challengeData.challenge_id}
        />
        <hr className="m-0" />
        {isLoggedIn && participate === 1 && status > 0 ? (
          <DetailNav status={status} tab={navTab} setTab={setNavTab} />
        ) : (
              ...
              <div className="flex-1 flex px-5 justify-between">
                <div className="flex-none flex gap-3">
                  ...
                  <div className="flex-none">
                    {status == 0 && dayDiff > 0 && (
                      <div className="cd-due-date">D-{dayDiff}</div>
                    )}
                  </div>
                </div>
                {status == 0 &&
                  user_nickName == challengeData.leader_nickname &&
                  challengeData.now_people == 1 && (
                    <div className="flex-none flex gap-1">
                      <button
                        className="flex-none rounded-md text-[13px] font-semibold px-4 mt-[3px] my-[2.75px] py-[0.5px] border-solid border-[#6E6053] border-[2px]"
        ...
        {navTab == "info" ? (
          <ChallengeFooter
            ...
            participant_list={challengeData.participant_list}
            ...
            onClose={setCancelChallModal}
            onClose2={setQuitChallModal}
          />
        ) : (
          ""
        )}
      ...
      {!isLoggedIn && requestLogin ? (
        <RequestLogin
          onClose={setRequestLogin}
          purpose="참가"
          prevPage={`challenge/${id}`}
        />
      ) : (
        ""
      )}
      ...
      {cancelChallModal && (
        <CancelChallModal
          onClose={setCancelChallModal}
          cancelChallID={id}
          cancelChallName={challengeData.challenge_name}
        />
      )}
      {quitChallModal && (
        <QuitChallModal
          onClose={setQuitChallModal}
          cancelChallID={id}
