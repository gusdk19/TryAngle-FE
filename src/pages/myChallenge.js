...
export default function MyChallenge() {
  ...
  const { isLoggedIn, login, logout, user_token, user_nickName } =
    useAuthStore();
  ...
  useEffect(() => {
    const getAuthList = async (challengeId) => {
      try {
        ...
        console.log(
          "find each Authentication list check",
          data,
          data.isSuccess,
          data.message
        );
      ...
      try {
        ...
        if (data.isSuccess) {
          const updatedResult = await Promise.all(
            data.result.map(async (challenge) => {
              ...
              return {
                ...
                auth_status: getData?.auth_image ? true : false,
              };
            })
          );
      ...
      <div className="bg-white w-[393px] h-[852px] relative">
        ...
        {isLoggedIn ? (
          <>
            ...
            {tab === "onProgress" && (
              <OnProgressChall
                dueChallengeList={dueChallengeList}
                onProgressChallengeList={onProgressChallengeList}
            ...
            {tab === "create" && (
              <CreateChall
                leaderChallengeList={leaderChallengeList}
                setChallengeList={setChallengeList}
                user_nickName={user_nickName}
              />
            )}
