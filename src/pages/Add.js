...
export default function Add() {
  ...
  const handleSubmit = async (e) => {
    ...
    const challengeData = {
      ...
      start_date: `${startYear}-${String(startMonth).padStart(2, "0")}-${String(
        startDay
      ).padStart(2, "0")}`,
    ...
    formData.append(
      "leaderJoinData",
      new Blob([JSON.stringify(leaderJoinData)], { type: "application/json" })
    );
    if (imageFile) {
      formData.append(
        "thumbnailImage",
        bI ? fetchImageAsFile(profileImage) : imageFile
      ); // ✅ 이렇게 전송
    }
