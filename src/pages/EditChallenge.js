...
export default function EditChallenge() {
  ...
  const [challengeName, setChallengeName] = useState(
    updatedChallenge.challenge_name || ""
  );
  const [challengeNameError, setChallengeNameError] = useState(false);

  const catNum =
    updatedChallenge.category == "ETC"
      ? 4
      : updatedChallenge.category == "WORKOUT"
      ? 1
      : updatedChallenge.category == "LIFE"
      ? 3
      : 2;
  const [category, setCategory] = useState(
    updatedChallenge.category ? catNum : ""
  );
  const [categoryError, setCategoryError] = useState(false);

  const [shortIntro, setShortIntro] = useState(
    updatedChallenge.challenge_shortintro || ""
  );
  ...
  const [certifyPeriod, setCertifyPeriod] = useState(
    updatedChallenge.auth_frequency || ""
  );
  ...
  const [profileImage, setProfileImage] = useState(
    updatedChallenge.challenge_thumbnail || ""
  );
  ...
  const handleSubmit = async (e) => {
    ...
    if (hasError) {
      ...
      setUpdatedChallenge(async (prev) => ({
        ...
        category: category,
        start_date: `${startYear}-${String(startMonth).padStart(
          2,
          "0"
        ...
        navigate(`/challenge/${challenge.challenge_id}`, {
          state: {
            challenge: {
              ...
              end_date: `${endYear}-${String(endMonth).padStart(
                2,
                "0"
    ...
    <div className="bg-white flex justify-center w-full">
      ...
      <div className="bg-white w-[393px] h-[852px]">
        ...
        <main className="main h-[730px] overflow-auto px-5 mt-[20px] pb-6">
          <h2 className="text-[24px] font-bold mb-4 text-[#4A483F]">
            챌린지 정보 수정
          </h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              <FieldError
                label="10글자 소개"
                error={
                  shortIntroError ? "10글자 이내로 소개글을 입력해주세요" : ""
                }
              />
              <input
                type="text"
              ...
              />
              <p className="px-1 mt-2 text-sm text-[#B3B3B3]">
                ※ 짧은 소개글은 섬네일에 표시됩니다
              </p>
            </div>
            {/*챌린지 기간*/}
            <div className="mb-6">
              ...
              <div className="scroll overflow-x-auto overflow-y-hidden whitespace-nowrap rounded px-2 py-1 flex gap-2">
                ...
                <select
                  ...
                  className={`border w-[85px] px-2 py-1 rounded text-sm ${
                ...
                <select
                  ...
                  className={`border w-[60px] px-2 py-1 rounded text-sm ${
              ...
              <div className="scroll overflow-x-auto overflow-y-hidden whitespace-nowrap rounded p-1 flex gap-2">
                {CERTIFY_OPTIONS.map((option) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setCertifyPeriod(option);
                      setCertifyPeriodError(false);
                    }}
            ...
            <div className="mb-6">
              <FieldError
                label="최대 모집 인원"
                error={
                  maxParticipantError ? "최대 모집 인원을 선택해주세요" : ""
                }
              />
              <select
            ...
            <div className="mb-6">
              <FieldError
                label="챌린지 비용"
                error={
                  depositTypeError ? "챌린지 비용 방식을 선택해주세요" : ""
                }
              />
              {/* 선택 버튼(예치금/기부) */}
              ...
              {!choiceBasicImage ? (
                <div className="w-full h-[190px] bg-white mx-auto mt-[10px] rounded-lg">
                  <div className="grid items-center w-full h-full">
                    <div
                      className={`relative m-auto w-[150px] h-[145px] border border-gray-300 rounded-md flex flex-col justify-center items-center gap-2`}
                    >
                      {profileImage && (
                        <div className="absolute z-10">
                          <img
                            ...
                            className={`w-[150px] ${
                              profileImage
                                ? "h-fit max-h-[150px] opacity-[80%]"
                                : "h-[145px]"
                            } object-fill rounded-md`}
                          />
                        </div>
                      )}
                      ...
                      >
                        <span className=" w-[95px] h-[24px] border border-gray-300 rounded-md flex flex-col justify-center items-center upload-image-btn text-sm text-[#B3B3B3] ">
                          사진 업로드
                        </span>
                      </label>
                      ...
                      {basicImages.map((img, idx) => (
                        <img
                          ...
                          className={`w-[56px] h-[56px] cursor-pointer object-cover ${
                            prePI === img
                              ? "border-[3px] border-[#4A483F] rounded-[15px]"
                              : ""
                          }`}
                          onClick={() => {
                            setPrePI(img);
            ...
            <button
              type="submit"
              className="w-[272px] ml-10 mt-[-8px] mb-[30px] py-3 text-white font-semibold rounded-lg bg-[#FAB809] transition duration-200 shadow-sm"
            >
              챌린지 수정
            </button>
          </form>
