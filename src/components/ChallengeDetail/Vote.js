...
export default function Vote({challengeID}){
      ...
      const getVoteStatusList = async()=>{
        ...
        try {
          ...
          if(data.isSuccess){
            ...
            const updatedResult = await Promise.all(data.result.map(async(user, idx) => {
                // 해당 challenge_id와 일치하는 authData 항목 찾기
                
                const matchingAuth = getData.find((data)=>(data.user_nickname == user.nickname))
                ...
                return {
                    ...
                    auth_image: matchingAuth?.auth_image ? matchingAuth?.auth_image : null,
                    voter_id : idx,
    ...
    const postEmotion = async (icon) => {
      ...
      try {
          const res = await fetch(`http://localhost:8080/authentication/${authenticationId}/reaction`, {
              method: 'POST',
              headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${user_token}`
              ...
              body: JSON.stringify({ 
                  "reaction_id": icon == "heart" ? 0 : icon == "doubt" ? 1 : icon == "cheerup" ? 2 : 3
              }), 
          });
    ...
    return(
        <div className="px-5">
            ...
            {onVote === "" ? "" : <VoteFooter onVoteUser={onVoteUser} onVoteUserID={onVoteUser.voter_id} setVoteStatusList={setVoteStatusList} setOnVote={setOnVote}/>}
