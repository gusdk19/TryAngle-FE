...
const editAuth = async (
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/authentication/${authenticationId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${user_token}`,
...
export default function Vertify({ vertifyMethod, challengeId }) {
  ...
  useEffect(() => {
    const getAuthList = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/authentication/my/${challengeId}`,
          {
            method: "GET",
            headers: {
        ...
        console.log(
          "find each Authentication check",
          data,
          data.isSuccess,
          data.message
        );
  ...
  const capture = () => {
    ...
    const authData =
      authImage == ""
        ? { challenge_id: challengeId, comment: `챌린지 ${challengeId} 인증` }
        : { comment: `챌린지 ${challengeId} 인증` };

    const formData = new FormData();
    formData.append(
      "authData",
      new Blob([JSON.stringify(authData)], { type: "application/json" })
    );
    if (imageSrc) {
  ...
  return (
    <div className="mt-3">
      {loading ? (
        ...
        authImage ? (
          <>
            <div className="grid grid-center h-[260px] px-auto bg-transparent rounded-lg">
              <img
                className="overflow-auto mx-auto rounded-md max-w-full max-h-full"
                src={authImage}
              />
            </div>
            <div className="flex justify-center mt-2 px-auto">
              <button
                className={`flex-none mx-auto font-medium text-[#d9d9d9] px-3 py-1 
          ...
          <>
            <div className="grid grid-center h-[260px] bg-[#D9D9D9] rounded-lg">
              <HiOutlineCamera
                className="m-auto w-[48px] h-[48px] text-white cursor-pointer"
                onClick={() => {
                  setCloseModal(1);
                }}
              />
        ...
        <>
          {vtMethod == "camera" ? (
            authImage ? (
              <div
                className="grid grid-center h-[260px] px-auto bg-transparent rounded-lg"
              ...
              <>
                ...
                />
                <FaRegCircle
                  className="w-[40px] h-[40px] m-3 mx-auto text-[#D9D9D9] cursor-pointer"
                  onClick={capture}
                  hidden={vtMethod == "camera" ? false : true}
                />
              </>
            ...
            <>
              ...
              {authImage ? (
                ...
                <div
                  className="grid grid-center h-[260px] bg-[#D9D9D9] rounded-lg"
                  hidden={vtMethod == "album" ? false : true}
                >
                  <BiPhotoAlbum
                    className="m-auto w-[48px] h-[48px] text-white cursor-pointer"
      ...
      {closeModal ? (
        <VertifyMethodModal onClose={setCloseModal} setVtMethod={setVtMethod} />
      ) : (
        ""
      )}
    </div>
