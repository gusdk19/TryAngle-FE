        ...
        <div className="modal-sq w-full h-full">
          ...
          {errors.length == 0 ? (
            <div className="mx-auto my-[15px] flex flex-col justify-between gap-2 text-center">
              <span
                htmlFor="withdrawal"
                className="ml-[2px] mt-[2px] flex-none text-[#6e6053] text-[16px]"
              >
                <span className="flex flex-col gap-2">
                  <span>
                    임시 비밀번호가 이메일로{" "}
                    {resend ? (
                      <span className="font-bold">재전송</span>
                    ) : (
                      "전송"
                    )}
                    되었습니다.
                  ...
                  <span>
                    (임시 비밀번호 :{" "}
                    <span className="font-bold text-[red]">{tempPW}</span>)
                  </span>
                  <span>
                    이메일 확인 후, <span className="font-bold">확인 버튼</span>
                    을 눌러 주시기 바랍니다.
                  </span>
                  <span className="text-[red] text-[15px]">
                    ※ 이메일이 전송되지 않았을 경우, 아래{" "}
                    <span className="font-bold">재전송 버튼</span>을 눌러주시기
                    바랍니다.
                  </span>
                </span>
              </span>
          ...
          <div className="w-full flex flex-row gap-2">
            {errors.length == 0 ? (
              <button
                className={`flex-1 mx-auto h-[38px] mt-[2px] mb-[5px] py-1 rounded-md 
