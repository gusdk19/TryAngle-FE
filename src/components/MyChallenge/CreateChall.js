...
export default function CreateChall({leaderChallengeList}){
    ...
    const [filter, setFilter] = useState("recruiting"); // filter(2) : "recruiting" - 모집중, "recruit-complete" - 모집완료
        ...
        <div className='w-full px-5 mt-4'>
            ...
            <div className='main h-[619px] pb-4 overflow-scroll'>
                ...
                <div className='card-container'>
                    {(filter == "recruiting" ? recruitingChallengesSorted : recruiteCompleteChallengesSorted).map((challenge)=>{
                        const now = new Date();
                                ...
                                <div className='card-text flex flex-row gap-[2px]'>
                                    ...
                                    {filter == "recruiting" &&
                                     <button className={`number-participants `}>
                                        신청인원(<span className='text-[#B8AA96]'>{challenge.now_people}</span>/{challenge.max_people})
                                    </button>}
                                </div>
                            </div>
                    )})}
