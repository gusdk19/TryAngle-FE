...
export default function AddFriend({
  ...
  const handleFollow = (targetUser, following) => {
    ...
    setFollowings((prevFollowings) => {
      const exists = prevFollowings.some(
        (following) => following.userId === targetUser.userId
      );
      if (exists) {
        // 삭제
        return prevFollowings.filter(
          (following) => following.userId !== targetUser.userId
        );
      } else {
        //추가
        ...
        return (
          ...
          >
            <img
              ...
              alt={`${user.userId}-profileImage`}
            />
            <span className="flex-1 my-auto text-[15px] text-[#6E6053] font-medium">
              {user.nickname}
            </span>
            <button
              className={`follow-cancel-btn flex-none h-fit py-[5px] my-auto text-[13px] font-medium  text-[#4A483F] ${
                user.isFollowing ? "bg-[#F4F4F4] px-5" : "bg-[#FAB809]  px-8"
              } rounded-lg`}
              onClick={() => {
                handleFollow(user, user.isFollowing);
              }}
            >
