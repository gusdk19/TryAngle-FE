...
export default function ProfileEditModal({
  ...
  const handleInputChange = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/user/checkNickname`, {
        method: "POST",
        headers: {
  ...
  const handleChange = async () => {
    ...
    try {
      ...
      const data = await res.json();
      console.log(
        "modify nickname & profileImage check",
        data.isSuccess,
        data.message
      );
    } catch (error) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/user/modify/description`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user_token}`,
          ...
          {!choiceBasicImage ? (
            ...
            <>
              ...
              <div className="mx-auto mt-[30px] rounded-lg w-[192px] h-[192px] bg-white">
                <div className="profile-image grid items-center w-full h-full">
                  <div className="image-btn ">
                    <button
                      className={`basic-image-btn h-fit`}
                      onClick={() => {
                        setChoiceBasicImage(true);
                      }}
                    >
                      기본 이미지 선택
                    </button>
                    <label
                      htmlFor="imageUpload"
                      className="upload-button grid items-center cursor-pointer"
                    >
              ...
              <div className="mx-auto px-4 mt-[25px] flex flex-row justify-between gap-2">
                <label
                  htmlFor="nickname"
                  className="text-[12px] mt-[2.5px] flex-none text-[#6e6053]"
                >
              ...
              <div className="w-full grid items-center">
                <button
                  className={`mx-auto w-[192px] h-[38px] mt-[25px] mb-[5px] px-2 py-1 rounded-md 
            ...
            <div className="mx-auto mt-[30px] rounded-lg w-[223px] p-[20px] bg-white">
              <div className="grid-container ">
                {basic_profile_image.map((img, idx) => (
                  <img
                    key={idx}
                    src={img}
                    alt={`basic-profile-image-${idx}`}
                    className={`grid-image w-[50px] h-[50px] 
                            ${
                              prePI == img
                                ? "border-[3px] border-[#4A483F] border-solid rounded-[15px]"
                                : ""
                            }`}
                    onClick={() => {
                      setPrePI(img);
              ...
              <div className="w-full grid items-center">
                <button
                  className={`mx-auto w-[192px] h-[38px] mt-[15px] mb-[5px] px-2 py-1 rounded-md 
                        text-white font-medium text-[15px] text-center items-center 
                        bg-[#FFC421] cursor-pointer`}
                  onClick={() => {
                    setProfileImage(prePI);
                    setChoiceBasicImage(false);
                  }}
                >
                  선택
                </button>
