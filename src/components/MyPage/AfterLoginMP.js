...
export default function AfterLoginMP({ logout, setUserNickName }) {
  ...
  const { following, follower, totalReturn, origUserData } =
    location.state || {};
  ...
  useEffect(() => {
    if (origUserData) {
      ...
      const getUserData = async () => {
        try {
          const res = await fetch(`${API_BASE_URL}/user/mypage`, {
            method: "GET",
            headers: {
          ...
          if (data.isSuccess) {
            ...
            setBadges([
              ...
              },
              {
                ...
                name: "챌린지 수호자",
                description:
                  "10명 이상의 다른 참여자에게 챌린지 권유 및 참여 유도(상대방이 권유 수락 시 인정)",
                unlockCondition:
                  "10명 이상의 다른 참여자에게 챌린지 권유 및 참여 유도(상대방이 권유 수락 시 인정)",
                isVisible: false,
                image: badge13,
      ...
      const fetchCalendarData = async (date) => {
        ...
        try {
          const res = await fetch(
            `${API_BASE_URL}/challenge/calendar?ym=${ym}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
  ...
  if (loading) {
    return (
      <div className="w-full h-[734px] grid items-center">
    ...
    <main className="main flex flex-col w-full h-[734px] px-5 overflow-scroll">
      ...
      <div className="flex items-start mt-5 mb-4">
        ...
        <div className="relative">
          ...
          >
            <MdOutlineEdit
              className="ml-[3px] h-[15px] w-[15px] stroke-[#4A483F] fill-[#4A483F]"
              onClick={() => {
                setShowPEModal(true);
              }}
            />
        ...
        <div className="h-full ml-5 flex flex-col justify-center gap-1">
          <div
            variant="outline"
            className="flex flex-row gap-5 mt-[6px] font-semibold text-[#4A483F] text-[15px]"
          >
            <div className="">{userData.nickname}</div>
            <button
              className="pb-[0.5px] font-normal underline text-[11px] text-[#838687]"
              onClick={() => {
                logout();
              }}
            >
          ...
          <div className="flex items-center gap-3 text-xs text-[#6E6053]">
            ...
            <hr
              ...
              className="h-[10px] mt-[4px] border-[#6E6053] border-l-[0.1px]"
            />
            <span>
              팔로잉
            ...
            >
              <UserPlus
                className="h-[13px] w-[13px] stroke-[#6E6053] stroke-[2px]"
                onClick={() => {
                  navigate("/friend", {
                    state: {
      ...
      <div className="w-full mb-4 bg-[#FDF8ED] border-none rounded-md">
        <div className="px-3 py-2 flex items-center">
          <span className="text-[#6E6053] text-[12px]">
      ...
      <UserDepositReward
        deposit={userData.challengeMoney}
        reward={userData.returnMoney}
      />
      ...
      <div className="mt-[23px] mb-4">
        ...
        <button
          className="text-xs text-[#6e6053] mt-1 ml-1"
      ...
      <div className="mt-[23px] mb-6">
        ...
        <div className="flex flex-col">
          <button className="text-xs text-[#6e6053] text-left mt-1 ml-1">
            1:1 문의
          </button>
          <button className="text-xs text-[#6e6053] text-left mt-1 ml-1">
            약관 및 정책
          ...
          <button
            className="text-xs text-[#6e6053] text-left mt-1 ml-1"
            onClick={() => {
              setShowSEModal(true);
            }}
          >
      ...
      {showPEModal && (
        <ProfileEditModal
          origNickname={userData.nickname}
          origPI={userData.profileImage}
          origDescription={userData.description}
          onClose={setShowPEModal}
          changeUserData={setUserData}
        />
      )}
      {showSEModal && (
        <SecedeModal
          onClose={setShowSEModal}
